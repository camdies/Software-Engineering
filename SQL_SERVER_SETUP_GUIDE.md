...
| semester_config | 学期配置表（默认20周、选课开关） |
| user_account | 用户账号（admin/teacher/student） |
| student | 学生信息（含年级、邮箱、联系方式） |
| teacher | 教师信息（含职称、邮箱、联系方式） |
| course | 课程信息（含课程类型、面向专业） |
| course_plan | 开课计划（教师申请制，自由选择起止周） |
| enrollment | 选课记录 |
| grade | 成绩记录（含 new_score 字段） |
| operation_log | 操作日志 |
