<template>
  <div class="page-card">
    ...
    <el-table :data="filteredPlans" stripe v-loading="loading">
      ...
      <el-table-column prop="time_slot" label="上课时间" min-width="220" />
    ...
    <el-dialog v-model="dialogVisible" :title="editing ? '修改申请' : '新增授课申请'" width="640px">
      <el-form ref="formRef" :model="form" label-width="80px">
        <el-form-item label="课程" required>
          <el-select v-model="form.course_id" filterable placeholder="搜索课程" style="width:100%" :disabled="!!editing"
        ...
        <el-row :gutter="12">
          ...
          <el-col :span="8">
            <el-form-item label="起始节" required>
              <el-select v-model="form.period_start" style="width:100%">
                <el-option v-for="p in 11" :key="p" :label="`第${p}节`" :value="p" />
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="节数" required>
              <el-select v-model="form.period_count" style="width:100%">
                <el-option v-for="c in 11" :key="c" :label="`${c}节`" :value="c" />
              </el-select>
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-form-item label="先修课"><el-input v-model="form.prerequisite" placeholder="课程代码，逗号分隔" /></el-form-item>
        <el-form-item label="申请理由">
          <el-input v-model="form.apply_reason" type="textarea" :rows="3" placeholder="请说明开设理由，供管理员审核参考" />
        </el-form-item>
        <el-form-item label="预览">
          <el-tag size="large">{{ previewTime }}</el-tag>
          <span style="font-size:var(--text-scale-2xs);color:var(--neutral-400);margin-left:var(--space-2)">
            提交后需管理员审核通过方可生效
          </span>
        </el-form-item>
      </el-form>
