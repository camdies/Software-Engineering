  ...
  <div class="page-card">
    <div class="page-toolbar">
      <div class="search-group">
        <span class="toolbar-hint">选课时段由 <strong>semester_config</strong> 表控制，每个学期可配置独立的选课时段</span>
      </div>
    ...
    <el-table :data="list" stripe v-loading="loading">
      ...
      <el-table-column label="学期日期" width="200">
        <template #default="{ row }">{{ row.start_date || '-' }} ~ {{ row.end_date || '-' }}</template>
      </el-table-column>
      <el-table-column label="当前学期" width="90">
        <template #default="{ row }">
          <el-tag :type="row.is_current ? 'success' : 'info'" size="small">{{ row.is_current ? '是' : '否' }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="选课开放" width="90">
        <template #default="{ row }">
          <el-tag :type="row.enrollment_open ? 'success' : 'danger'" size="small">{{ row.enrollment_open ? '开放' : '关闭' }}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="180" fixed="right">
        <template #default="{ row }">
          ...
          <el-button size="small" type="danger" @click="del(row.config_id)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>

    <el-dialog v-model="dialogVisible" :title="editing ? '编辑选课时段' : '新增选课时段'" width="520px">
      <el-form ref="formRef" :model="form" label-width="100px">
        ...
        <el-form-item label="设为当前学期">
          ...
          <span class="form-extra">设为当前学期会取消其他学期的"当前"标记</span>
        </el-form-item>
        <el-divider />
...
async function del(id) {
  try {
    await ElMessageBox.confirm('确定删除该选课时段配置？', '提示', { type: 'warning' })
  } catch { return }
  try {
...
.form-extra { font-size: 12px; color: #909399; margin-left: 8px; }
</style>
